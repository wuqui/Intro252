...
local function compile_tikz_to_svg(code, user_opts, conf, basename)  -- Added conf and basename parameters
  ...
  local function process_in_dir(dir)
    return with_working_directory(dir, function()
      ...
      local tikz_template = pandoc.template.compile [[\documentclass[tikz]{standalone}
% \usepackage{tikz} % already loaded by the documentclass
\usepackage[linguistics]{forest}
\usepackage{pagecolor}
\pagecolor{white}
% Custom sealing style for linguistics trees
\forestset{
  sealing/.append style = {
    grow'=east,
    anchor=west,
    calign=first,
    child anchor=west,
    parent anchor=east,
    font=\sffamily,
    where n children=0{edge=dotted, tier=branch, font={\rmfamily \itshape}, anchor=east}{},
...
local function code_to_figure(conf)
  return function(block)
    ...
    -- Save to custom svg-dir if configured, otherwise use mediabag
    if conf.svg_dir then
      local svg_path = pandoc.path.join { conf.svg_dir, fname }
      write_file(svg_path, imgdata)
      -- Create the image object with path relative to session directory.
      local image = pandoc.Image(dgr_opt.alt, svg_path, "", dgr_opt['image-attr'])
      return dgr_opt.caption and
          pandoc.Figure(
            pandoc.Plain { image },
...
local function configure (meta, format_name)
  ...
  -- Handle svg-dir option (custom output directory for SVGs)
  local svg_dir = nil
  if conf['svg-dir'] then
    svg_dir = pandoc.utils.stringify(conf['svg-dir'])
    pandoc.system.make_directory(svg_dir, true)
  end
