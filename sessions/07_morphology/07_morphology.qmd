---
title: 7 — Morphology and word-formation
subtitle: "Introduction to linguistics — [Course Website](../../index.qmd) — 1 & 8 November 2025"
author: Dr. Quirin Würschinger, LMU Munich
metadata:
  linkprefix: ../../
format:
  html:
    output-file: "website.html"
  clean-revealjs:
    output-file: "slides.html"
format-links: false
params:
  show_excl: true
---

::: {.content-hidden when-format="revealjs"}
[Open slides](slides.html){.btn .btn-primary}
:::


# Morphology

## Morphology and word-formation

![@Schmid2016Morphology: 15](att/morphology-vs-word-formation.png)

::: {.notes}
Do syntactic analysis.
:::


## Object of study

"Morphology is concerned with the internal structure of words and with the various processes which allow us to constantly expand the vocabulary of a language." [@Kortmann2020Essentials: 51] 


## Morphemes

"**Morpheme**: The basic morphological unit is not the word, but the morpheme (Greek morphé = shape, form), the smallest meaning-bearing unit of language. 

Thus, the word *singers* contains three morphemes: {sing}, {-er}, and {-s}. Each of these three morphemes adds to the overall meaning of singers: 

- the verb {sing} makes the central contribution, 
- while {-er} on its own means no more than 'someone who VERBs', 
- and the {-s} merely gives grammatical information, namely plural.” 

[@Kortmann2020Essentials: 51]


## Inflectional morphology

![@Kortmann2020Essentials: 81](att/inflectional-morphology.png)


## Allomorphs 

- "**Morphemes vs (allo-)morphs**: On the level of morphology, morphemes are the exact **counterpart to phonemes** on the level of phonology. 
- Just like phonemes, morphemes are abstract units which can be realized by more than one form. 
- Just think of the plural morpheme in word forms like *kids*, *kits*, and *kisses*, where it is realized as /-z/, /-s/, and /-ɪz/ respectively. 
- These **concrete realizations of morphemes are called morphs**, and in analogy to the allophones of a phoneme we speak of the allomorphs of a morpheme” 

[@Kortmann2020Essentials: 51]


## Word, word form, lexeme

- "**Word – word form – lexeme**: The example *singers* also serves to show how important it is to deal with the term word in a more differentiated way. 
- Is *singers* a different **word** from *singer*? No, singers is merely a different form, a so-called **word form**, of the noun *singer*. 
- *Singer* itself, on the other hand, is not merely a word form of *sing*, but a different, new word, which has been formed by affixation of {-er} to the verb. 
- In this case, we speak of a new **lexeme** with a new dictionary entry which has been created by a specific derivational process.” 

[@Kortmann2020Essentials: 51]


## Types of morphemes

![@Kortmann2020Essentials: 53](att/types-of-morphemes.png)

---

![@Schmid2016Morphology: 36](att/schmid-morpheme-types.png)

---

### Exercise: determine morpheme types

Divide the words below into their component morphemes and classify them.

1. *signpost*: [{sign}~lx.fr~ {post}~lx.fr~]{.excl .fragment}
2. *cats*: [{cat}~lx.bd~ {plural}~gr.bd~]{.excl .fragment}
3. *they*: [{they}~gr.fr~]{.excl .fragment}
4. *singer*: [{sing}~lx.fr~ {er}~lx.bd~]{.excl .fragment}
5. *finger*: [{finger}~lx.fr~]{.excl .fragment}
6. *funnier*: [{funny}~lx.fr~ {er}~gr.bd~]{.excl .fragment}
7. *undeniable*: [{un}~lx.bd~ {deny}~lx.fr~ {able}~lx.bd~]{.excl .fragment}


## Special types of morphemes

### Portmanteau morphemes

::: {.small}
- morphemes that carries several meanings simultaneously
- i.e. that carry information which is usually expressed by way of two or more morphemes
- e.g. *took*, *taught*
:::

![](att/portmanteau.excalidraw.svg)

---

### Zero morphemes

- an inflectional or word-formation morpheme lacking phonetic substance
- e.g. {plural} morpheme in *two sheep*

![](att/zero-morphemes.excalidraw.svg)


## Base, root, stem

Example: *removals*

- "What remains once all inflectional suffixes (here plural {-s}) are taken away is the **stem**. 
- What remains when taking away all affixes ({-re}, {-al} and {-s}) is the **root**, which is the minimal lexical unit and cannot be morphologically analysed any further. 
- What remains if individual derivational affixes are taken away from the stem (here: {-al} from removal) is called the **base** in the narrower sense - which is still larger than the root."

[@Kortmann2020Essentials: 54]

---

![](att/removals.excalidraw.svg)

---

### Exercise: determine base, root, stem

Determine the base, root, and stem of the word *formalities*.

::: {.excl .fragment}
![](att/formalities.excalidraw.svg)
:::


## Allomorphs

---

### Complementary distribution

![@Kortmann2020Essentials: 55](att/complementary-distribution.png){width=600}


---

### Plural allomorphs

![@Herbst2010English: 85](att/plural-allomorphs.png){width=600}

---

### Past tense allomorphs

![@Herbst2010English: 90](att/past-allomorphs.png){width=600}

---

### Overview of allomorphy in English

![@Schmid2016Morphology: 61](att/allomorphy-overview.png)

---

### Irregular allomorphs

![@Kortmann2020Essentials: 58](att/irregular-allomorphs.png)

---

### Phonological conditioning of allomorphs

#### Types

- “**affix** allomorphy, where a particular free morpheme, i. e. base, demands the choice of a particular (form of a) bound morpheme, 
- and **base** allomorphy, where, vice versa, a particular affix (be it derivational or inflectional) demands the choice of a particular form of the base.” 

[@Kortmann2020Essentials: 58]

---

#### Examples

![@Kortmann2020Essentials: 59](att/allomorphy-examples.png){width=600}

**(6)**: Consonant change of the final sound of the base due to **coarticulation**.\
**(7)**: Vowel changes of the base due to **stress shift** triggered by derivation.

---

### Exercise: analyse plural allomorphs

In which way do the plural forms of the following nouns differ?

*cat*, *horse*, *woman*, *mouse*, *dog*, *book*, *fish*, *orange*, *apple*, *car*, *child*, *tax*, *ox*, *cliff*, *deer*

**Options:**

1. `/s/`
2. `/z/`
3. `/ɪz/`
4. irregular

::: {.excl .slide}
| 1. /s/    | 2. /z/   | 3. /ɪz/   | 4. irregular |
|:---------:|:--------:|:---------:|:------------:|
| *cats*    | *dogs*   | *horses*  | *women*      |
| *books*   | *apples* | *oranges* | *fish*       |
| *cliffs*  | *cars*   | *taxes*   | *children*   |
|           |          |           | *oxen*       |
|           |          |           | *deer*       |
|           |          |           | *mice*       |
:::


## Morphological language types

![@Kortmann2020Essentials: 85](att/morphological-language-types.png)


## Morphology Review Quiz (Interactive)

```{=html}
<div style="width:100%;display:flex;flex-direction:column;gap:8px;min-height:635px;">
<iframe src="https://wayground.com/embed/quiz/692cacc0557a086a79635dd4" title="Morphology Review Quiz - Wayground" style="flex:1;" frameBorder="0" allowfullscreen></iframe>
</div>
```

# Word-formation

## Morphemic word-formation

![](att/morphemic-wf-overview.excalidraw.svg)


## Derivation

---

### Derivation vs inflection

![@Kortmann2020Essentials: 86](att/inflection-vs-derivation.png){height="500px"}

---

### Prefixation


::: {.columns}
:::: {.column width="50%"}
![](att/unhappy.excalidraw.svg)
::::
:::: {.column width="50%"}
- **formal structure**
  - prefix: {A}~lx.bd~
  - base: {B}~lx.fr~
- **function**
  - modification of meaning
  - usually word-class preserving (exceptions: *asleep*, *enlarge*)
::::
:::

---

#### Morphemes

![](att/prefixation-morphemes.png){width=600}

---

### Suffixation

::: {.columns}
:::: {.column width="50%"}
![](att/happiness.excalidraw.svg){width=100%}
::::
:::: {.column width="50%"}
- **formal structure**
  - base: {A}~lx.fr~
  - suffix: {B}~lx.bf~
- **function**
  - (abstract) modification in meaning (e.g. concept type)
  - often word-class changing (exceptions: *greenish*, *kingdom*)
::::
:::

----

#### Examples

![@Kortmann2020Essentials: 61](att/suffixation-examples.png){width=600}

---

## Compounding

::: {.columns}
:::: {.column width="50%"}
![](att/washing-machine.excalidraw.svg){width=100%}
::::
:::: {.column width="50%" .small}
**Endocentric compounds**

- **semantic structure**: ‘a certain type of machine: for washing’
- **head**: *machine*
  - determines word class: *machine*^n^ > *washing machine*^n^
  - stem for inflectional suffixes: *washing machines*, not *\*washings machine*
- **modifier**: *washing*
  - qualifies head semantically
  - often carries primary stress
::::
:::

---

### Examples

![@Kortmann2020Essentials: 64](att/compound-examples.png){width=600}

---

### Semantic types

::: {.smaller}
- **endocentric** compounds 
  - **structure**: A + B denotes a special kind of B
  - **examples**: *darkroom*, *small talk*
- **exocentric** compounds 
  - **structure**: A + B denotes a special kind of an unexpressed semantic head (e. g. ‘person’ in the case of *skinhead* or *paleface*)
  - these compounds often have metonymic character, when one part stands for the whole (e. g. *paleface* for ‘a person with a pale face’)
  - **examples**: *egghead*, *blockhead*, *blackhead*, *birdbrain*, *redneck*, *greenback*, *paperback* 
- **appositional** compounds 
  - **structure**: A and B provide different descriptions for the same referent
  - **examples**: *actor-director*, *actor-manager*, *writer-director*, *poet-translator*, *maidservant* 
- **copulative** compounds 
  - **structure**: A + B denotes ‘the sum’ of what A and B denote, i. e. the denotation of a copulative compound would be incomplete with one of the two elements A and B missing
  - **examples**: *spacetime*, *tractor-trailer*, *deaf mute*, *sleepwalk*, *freeze-dry* 

[@Kortmann2020Essentials: 65]
:::

---

### Exercise

Use the following terms to describe the meaning relationship between the component parts of the compounds below: 

> endocentric — exocentric — appositional — copulative compound

1. *greenhouse*: [endocentric]{.excl .fragment}
2. *knee-deep*: [endocentric]{.excl .fragment}
3. *pot head*: [exocentric]{.excl .fragment}
4. *Alsace-Lorraine*: [copulative]{.excl .fragment}


## Non-morphemic word-formation

### Overview

![](att/non-morphemic-wf-overview.excalidraw.svg){width=100%}

---

::: {.small}
| Process | Features | Examples |
|:---|:---------|:---|
| **Clipping** | parts of words are deleted without a change in meaning | *bike*, *exam*, *fridge* |
| **Blend** | forms and meanings of words are merged | *smog*, *brunch* |
| **Acronym** | shortened form retaining the initial letters of compounds or other fixed sequences of words; pronounced as words | *NATO, laser, AIDS* |
| **Alphabetism** | shortened form retaining the initial letters of compounds or other fixed sequences of words; pronounced as sequences of letters | *CNN*, *OED*, *USA* |
| **Back-formation** | word-class-changing word-formation process which deletes a morpheme or morpheme-like element; longer form pre-existing | *babysitter*^n^ > *babysit*^v^ |
| **Reduplication** | rare word-formation process repeating a word or word-like element either identically or in a slightly varied form | *walkie-talkie*, *hip-hop* |

: {tbl-colwidths="[15, 65, 20]"}
:::

---

### Shortenings

![@Kortmann2020Essentials: 72](att/shortenings.png)


## Differences between morphemic and non-morphemic word-formation

| Feature | Morphemic WF | Non-morphemic WF | Examples |
|:--------|:----------|:--------------|:---------|
| **Building blocks** | morphemes | parts of words | *br*-*unch* |
| **Combination** | adding morphemes | removing parts | in-*flu*-*enza* |
| **Serialization** | concatenative | interspersed | *CHuckLE x snORT* |
| **Regularity** | clear rules, predictable patterns | heterogeneous, high variability | *Br-angelina* |
| **Cognitive processing** | largely unconscious | conscious coining | *BBC* |


## Productivity

The degree to which a certain word-formation process is being employed to form new words.

### Examples

::: {.columns}
:::: {.column width="50%"}
**Still productive**: suffixation with **{ness}**: 

- {kind}{ness}
- {happy}{ness}
- {brave}{ness}
- …
::::
:::: {.column width="50%"}
**No longer productive**: suffixation with **{th}**: 

- {strong}{th} > strength
- {long}{th} > length
- {wide}{th} > width
- …
::::
:::


## Transparency

The degree to which there is a clear (transparent) mapping between the form and function/meaning of linguistic units.

### Examples

::: {.columns}
:::: {.column width="50%"}
**transparent**: 

- *player*: ‘somebody who plays’
- *wine bottle*: ‘a bottle of wine’
::::
:::: {.column width="50%"}
**intransparent/opaque**: 

- *caretaker*: *‘somebody who takes care’
- *undertaker*: *\‘somebody who undertakes sth.’
- *to kick the bucket*: *\‘to strike a bucket with your foot’
::::
:::


# Morphological analysis

## *singer-songwriter*

::: {.excl}
:::: {.r-stack}
![](att/singer-songwriter_1.excalidraw.svg){.fragment}

![](att/singer-songwriter_2.excalidraw.svg){.fragment}

![](att/singer-songwriter_3.excalidraw.svg){.fragment}

![](att/singer-songwriter_4.excalidraw.svg){.fragment}

![](att/singer-songwriter_5.excalidraw.svg){.fragment}

![](att/singer-songwriter_6.excalidraw.svg){.fragment}
::::
:::


## *misinterpretations*

::: {.excl}
:::: {.r-stack}
![](att/misinterpretations_1.excalidraw.svg){.fragment}

![](att/misinterpretations_2.excalidraw.svg){.fragment}

![](att/misinterpretations_3.excalidraw.svg){.fragment}

![](att/misinterpretations_4.excalidraw.svg){.fragment}
::::
:::


## *friendships*

::: {.excl}
:::: {.r-stack}
![](att/friendships_1.excalidraw.svg){.fragment}

![](att/friendships_2.excalidraw.svg){.fragment}

![](att/friendships_3.excalidraw.svg){.fragment}
::::
:::


## *campaigner*

::: {.excl}
:::: {.r-stack}
![](att/campaigner_1.excalidraw.svg){.fragment}

![](att/campaigner_2.excalidraw.svg){.fragment}

![](att/campaigner_3.excalidraw.svg){.fragment}
::::
:::


## Determine the word-formation process I

Analyze the word-formation processes involved in the following complex words.

1. *childhood*: [derivation / suffixation]{.excl .fragment}
2. *paperback*: [compounding / exocentric]{.excl .fragment}
3. *study-bedroom*: [compounding / copulative]{.excl .fragment}
4. *foreigner*: [derivation / suffixation]{.excl .fragment}
5. *paleface*: [compounding / exocentric]{.excl .fragment}
6. *Oxbridge*: [blending]{.excl .fragment}
7. *CGEL*: [initialism]{.excl .fragment}


## Exercise: determine the word-formation process II

By means of which **word-formation process** do we arrive at the following lexemes?[^1]

1. *to enthuse*: [**back-formation** from *enthusiasm*: N>V]{.excl .fragment}  
2. *laser*: [**acronymisation** from *Light Amplification by Stimulated Emission of Radiation*]{.excl .fragment}  
3. *lab*: [**clipping** from *laboratory* N>N]{.excl .fragment}  
4. *Hinglish*: [**blending** of *Hindi*^n^ and *English*^n^]{.excl .fragment}  
5. *BBC*: [**alphabetism** of *British Broadcasting Corporation*]{.excl .fragment}  
6. *sext*: [**blending** of *sex*^n^ and *text*^n^]{.excl .fragment}  
7. *to hoover*: [**(N) coinage** after American industrialist H. W. Hoover (1849–1932), then **conversion** N>V]{.excl .fragment}

[^1]: When two or more processes are involved specify their order.


# References

::: {#refs}
:::
